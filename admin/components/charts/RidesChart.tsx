...
export default function RidesChart({ data, loading = false }: RidesChartProps) {
  if (loading) {
    return (
  ...
  return (
    <div className="card p-5">
      <div className="mb-5">
        <h3 className="text-sm font-semibold text-gray-900">Rides per Day</h3>
        ...
        <AreaChart data={data} margin={{ top: 4, right: 0, left: -24, bottom: 0 }}>
          <defs>
            <linearGradient id="ridesGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="5%" stopColor="#1B9E77" stopOpacity={0.15} />
              <stop offset="95%" stopColor="#1B9E77" stopOpacity={0} />
            </linearGradient>
          </defs>
          ...
          <Area
            ...
            fill="url(#ridesGrad)"
            dot={false}
            activeDot={{ r: 4, fill: '#1B9E77', strokeWidth: 0 }}
          />
        </AreaChart>
      </ResponsiveContainer>
    </div>
